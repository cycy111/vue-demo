...
import { getErpOutStorageTotal } from '@/api/dianwangManagement/storageOutBackTotal';
import '@/styles/custom.scss';
export default {
  name: 'storageOutBackTotal',
  data() {
    return {
      useDepartmentData: [],
      currentPage: 1,
      ...
      getErpOutStorageTotal({
        ...
        .then(res => {
          if (res.data.resultCode == 1) {
            ...
            let cols =
              '借料单号,物资编号,物资名称,库存单位,计划数量,已发数量,总核销数量,erp出库未领数量,总退回数量,审核时间';
            let fields =
              'lendOrder,productCode,productName,productUnit,partQty,dliveryQty,sumQty,returnQty,sumRQty,checkDate';
