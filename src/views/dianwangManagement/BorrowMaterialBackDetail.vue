...
import { getCOutStorageDt } from '@/api/dianwangManagement/borrowMaterialBackDetail';
import '@/styles/custom.scss';
export default {
  name: 'borrowMaterialBackDetail',
  data() {
    return {
      useDepartmentData: [],
      currentPage: 1,
      ...
      getCOutStorageDt({
        ...
        .then(res => {
          if (res.data.resultCode == 1) {
            ...
            let cols =
              '审核时间,借料单号,项目编号,项目名称,施工单位名称,借料人,退料人,来源单号,物资编号,物资名称,库存单位,操作类型,总领用数量,本次核销数量,本次退回数量,总核销数量,上架储位';
            let fields =
              'checkDate,lendOrder,projectCode,projectName,enComName,lUser,cUser,docEntry,productCode,productName,productUnit,cType,getQty,cQty,rQty,sumQty,location';
