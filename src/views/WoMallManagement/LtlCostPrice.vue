  ...
  <div class="app-container">
    <div ref="searchForm" class="el-container-custom">
      <el-form
        ...
        size="small"
      >
        <el-form-item v-show="false" prop="costId">
          <el-input
    ...
    >
      ...
      <el-table-column prop="taxPrice" label="重量价格（含税）" width="120">
        <template slot-scope="scope">
      ...
      <el-table-column prop="price" label="重量价格（不含税）" width="120">
        <template slot-scope="scope">
    ...
    >
      ...
      >
        ...
        <el-form-item label="重量价格（含税）" v-show="true" prop="taxPrice">
          <el-input
        ...
        <el-form-item label="重量价格（不含税）" v-show="true" prop="price">
          <el-input
...
import {
  ...
  importPricesLd,
  getFleetList
} from '@/api/WoMallManagement/ltl-cost-price';
      ...
      getCpriceLd(data)
        .then(res => {
          if (res.data.resultCode == 1) {
            let cols =
              '成本价格表名称,承运商名称,始发省名称,始发市名称,目的省名称,目的地市名称,流向,区域类型,预计最少天数,预计最多天数,最低一票价格（含税）,最低一票价格（不含税）,重量价格（含税）,重量价格（不含税）,价保费率,税率,审核状态,起始有效期,结束有效期,备注,创建时间,修改时间,操作人';
            let fields =
              'priceName,fleetName,sendProvinceName,sendCityName,receiveProvinceName,receiveCityName,direction,areaTypeDesc,effectMinTime,effectMaxTime,minHeavyTaxPrice,minHeavyPrice,taxPrice,price,protectedPriceRate,tax,checkStateDesc,startDate,endDate,remark,addDate,editDate,operatorId';
