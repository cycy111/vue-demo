...
import {
  ...
  delConfirmSend,
  addQueueList
} from '@/api/WoMallManagement/shipment-confirm';
...
export default {
  ...
  methods: {
      ...
      getConfirmSend(data)
        .then(res => {
          if (res.data.resultCode == 1) {
            let cols =
              ' 物流单号,货物揽收时间,预计到货时间,备注,推送状态,创建时间,修改时间,操作人';
            let fields =
              'orderNO,collectTime,arriveTime,remark,sendStateDesc,addDate,editDate,operatorId';
    ...
    handleCreateBtnClickFunc() {
      ...
      this.dialog.title = '新增确认发货信息';
      this.dialog.status = 0;
      this.addForm.config.disabled = false;
    },
    submitAddData(data) {
      addConfirmSend(data).then(res => {
        if (res.data.resultCode === 1) {
