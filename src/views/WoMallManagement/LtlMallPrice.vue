...
import {
  ...
  addQueueList,
  getFleetList
} from '@/api/WoMallManagement/ltl-mall-price';
      ...
      getPriceApplyLd(data)
        .then(res => {
          if (res.data.resultCode == 1) {
            let cols =
              '申请单号,销售价格明细ID,始发省名称,始发市名称,目的省名称,目的地市名称,流向（如：济南-青岛）,区域类型,预计最少天数,预计最多天数,最低一票成本价格（含税）,最低一票成本价格（不含税）,最低一票销售价格（含税）,最低一票销售价格（不含税）,重量成本价格（含税）,重量成本价格（不含税）,重量销售价格（含税）,重量销售价格（不含税）,价保费率,税率,承运商名称,操作类型,起始有效期,结束有效期,备注,创建时间,修改时间,操作人';
            let fields =
              'applyNO,saleId,sendProvinceName,sendCityName,receiveProvinceName,receiveCityName,direction,areaTypeDesc,effectMinTime,effectMaxTime,cMinHeavyTaxPrice,cMinHeavyPrice,minHeavyTaxPrice,minHeavyPrice,cTaxPrice,cPrice,taxPrice,price,protectedPriceRate,tax,fleetName,operationTypeDesc,startDate,endDate,remark,addDate,editDate,operatorId';
