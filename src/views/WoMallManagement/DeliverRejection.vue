  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <el-form-item label="类型" v-show="true" prop="types">
          <el-select
            v-model="addForm.data.types"
            filterable
            placeholder="请选择类型"
            style="width:165px"
            clearable
          >
            <el-option
              v-for="(item, index) in selectArr.types"
        ...
        <el-form-item label="妥投人/拒收人联系方式" v-show="true" prop="mobile">
          <el-input
            v-model="addForm.data.mobile"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        <el-form-item label="妥投时间或拒收时间" v-show="true" prop="dTime">
          <el-date-picker
            v-model="addForm.data.dTime"
            type="datetime"
            style="width:165px"
            value-format="yyyy-MM-dd HH:mm:ss"
            placeholder="选择妥投时间或拒收时间"
          >
          </el-date-picker>
        </el-form-item>

        <el-form-item label="签收人" v-show="true" prop="signer">
          <el-input
            v-model="addForm.data.signer"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        <el-form-item label="签收人联系方式" v-show="true" prop="signMobile">
          <el-input
            v-model="addForm.data.signMobile"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        <el-form-item label="附件凭证下载路径" v-show="true" prop="attachment">
          <el-input
            v-model="addForm.data.attachment"
            placeholder=""
            style="width:165px;"
            clearable
          >
...
import {
  ...
  delSubmitDelivered,
  addQueueList
} from '@/api/WoMallManagement/deliver-rejection';
...
export default {
  ...
  components: {},
  watch: {
    'addForm.data.types': function(val, oldVal) {
      if (val == '0') {
        this.addForm.rules.mobile.push({
          required: true,
          trigger: 'blur',
          message: '妥投人联系方式不能为空'
        });
      } else {
        this.addForm.rules.mobile = [];
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleExportBtnClickFunc() {
      ...
      getSubmitDelivered(data)
        .then(res => {
          if (res.data.resultCode == 1) {
            let cols =
              '物流单号,类型,妥投人姓名或拒收人姓名,妥投人/拒收人联系方式,妥投时间或拒收时间,签收人,签收人联系方式,推送状态,附件凭证下载路径,备注,创建时间,修改时间,操作人';
            let fields =
              'orderNO,typesDesc,userName,mobile,dTime,signer,signMobile,sendStateDesc,attachment,remark,addDate,editDate,operatorId';
    ...
    handleCreateBtnClickFunc() {
      ...
      this.dialog.title = '新增妥投/拒收信息';
      this.dialog.status = 0;
      this.addForm.config.disabled = false;
    },
    submitAddData(data) {
      addSubmitDelivered(data).then(res => {
        if (res.data.resultCode === 1) {
    ...
    handleBatchDelBtnClickFunc() {
      ...
      delSubmitDelivered(this.selectIds)
        .then(res => {
          if (res.data.resultCode === 1) {
            this.searchFormMethod();
            Message({
    ...
    submitEditData(data) {
      upSubmitDelivered(data).then(res => {
        if (res.data.resultCode == 1) {
          this.searchFormMethod();
          Message({
            message: '修改成功',
