  ...
  <div class="app-container">
    <div ref="searchForm" class="el-container-custom">
      <el-form
        ...
        size="small"
      >
        <el-form-item v-show="true" prop="saleId">
          <el-input
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span v-show="scope.row.ftCostTaxPrice">
            <span class="col-content">{{ scope.row.ftCostTaxPrice }}</span
      ...
      >
        <template slot-scope="scope">
          <span v-show="scope.row.ftCostPrice">
            <span class="col-content">{{ scope.row.ftCostPrice }}</span
            ><span class="col-suffix">{{ '元' }}</span>
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="rtCostTaxPrice" label="续重成本价格" width="120">
        <template slot-scope="scope">
          <span v-show="scope.row.rtCostTaxPrice">
            <span class="col-content">{{ scope.row.rtCostTaxPrice }}</span
            ><span class="col-suffix">{{ '元' }}</span>
          </span>
        </template>
      </el-table-column>

      <el-table-column prop="rtCostPrice" label="续重成本价格" width="120">
        <template slot-scope="scope">
          <span v-show="scope.row.rtCostPrice">
            <span class="col-content">{{ scope.row.rtCostPrice }}</span
            ><span class="col-suffix">{{ '元' }}</span>
          </span>
        </template>
      </el-table-column>

      <el-table-column prop="reletTaxPrice" label="续重销售价格" width="120">
        <template slot-scope="scope">
      ...
      <el-table-column prop="reletPrice" label="续重销售价格" width="120">
        <template slot-scope="scope">
    ...
    >
      ...
      >
        ...
        <el-form-item label="快递销售价格表名称" v-show="true" prop="priceName">
          <el-input
        ...
        >
          <el-input
            v-model="addForm.data.ftCostTaxPrice"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        >
          <el-input
            v-model="addForm.data.ftCostPrice"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        <el-form-item label="续重成本价格" v-show="true" prop="rtCostTaxPrice">
          <el-input
            v-model="addForm.data.rtCostTaxPrice"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        <el-form-item label="续重成本价格" v-show="true" prop="rtCostPrice">
          <el-input
            v-model="addForm.data.rtCostPrice"
            placeholder=""
            style="width:165px;"
            clearable
          >
        ...
        <el-form-item label="续重销售价格" v-show="true" prop="reletTaxPrice">
          <el-input
        ...
        <el-form-item label="续重销售价格" v-show="true" prop="reletPrice">
          <el-input
...
import {
  ...
  addQueueList,
  getFleetList
} from '@/api/WoMallManagement/express-sales-price';
...
export default {
  ...
  methods: {
      ...
      getSpriceKd(data)
        .then(res => {
          if (res.data.resultCode == 1) {
            let cols =
              '唯一编号,销售价格表名称,成本价ID,承运商名称,始发省名称,始发市名称,目的省名称,目的地市名称,流向,区域类型,预计最少天数,预计最多天数,首重,首重成本价格（含税）,重量成本价格（不含税）,首重销售价格（含税）,首重销售价格（不含税）,续重成本价格,续重成本价格,续重销售价格,续重销售价格,价保费率,税率,备注,起始有效期,结束有效期,审核状态,推送状态, 申请单号,创建时间,修改时间,操作人';
            let fields =
              'saleId,priceName,costId,fleetName,sendProvinceName,sendCityName,receiveProvinceName,receiveCityName,direction,areaTypeDesc,effectMinTime,effectMaxTime,firstHeavy,ftCostTaxPrice,ftCostPrice,firstTaxPrice,firstPrice,rtCostTaxPrice,rtCostPrice,reletTaxPrice,reletPrice,protectedPriceRate,tax,remark,startDate,endDate,checkStateDesc,pushStateDesc,applyNO,addDate,editDate,operatorId';
        ...
        .then(res => {
          if (res.data.resultCode === 1) {
            ...
            for (let item of data) {
              ...
              if (
                !isNaN(item.ftCostTaxPrice) &&
                item.ftCostTaxPrice !== '' &&
                item.ftCostTaxPrice !== null
              ) {
                let ftCostTaxPrice = new Big(item.ftCostTaxPrice || 0);
                item.ftCostTaxPrice = parseFloat(ftCostTaxPrice.toFixed(4));
              }

              if (
                !isNaN(item.ftCostPrice) &&
                item.ftCostPrice !== '' &&
                item.ftCostPrice !== null
              ) {
                let ftCostPrice = new Big(item.ftCostPrice || 0);
                item.ftCostPrice = parseFloat(ftCostPrice.toFixed(4));
              }
              ...
              if (
                !isNaN(item.rtCostTaxPrice) &&
                item.rtCostTaxPrice !== '' &&
                item.rtCostTaxPrice !== null
              ) {
                let rtCostTaxPrice = new Big(item.rtCostTaxPrice || 0);
                item.rtCostTaxPrice = parseFloat(rtCostTaxPrice.toFixed(4));
              }

              if (
                !isNaN(item.rtCostPrice) &&
                item.rtCostPrice !== '' &&
                item.rtCostPrice !== null
              ) {
                let rtCostPrice = new Big(item.rtCostPrice || 0);
                item.rtCostPrice = parseFloat(rtCostPrice.toFixed(4));
              }
    ...
    handleCreateBtnClickFunc() {
      ...
      this.dialog.title = '新增快递线路销售价格查询';
    ...
    handleEditBtnClickFunc(row) {
      this.dialog.visible = true;
      this.dialog.title = '编辑快递线路销售价格查询';
      this.dialog.status = 1;
      this.addForm.config.disabled = true;
